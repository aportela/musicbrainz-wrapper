    ...
    public function get(string $mbId): \aportela\MusicBrainzWrapper\ParseHelpers\CoverArtArchiveHelper
    {
        ...
        if (! $this->getCache($mbId)) {
            ...
            if (!in_array($responseBody, [null, '', '0'], true)) {
                ...
                $this->logger->error(\aportela\MusicBrainzWrapper\CoverArtArchive::class . '::get - Error: empty body on API response', [$url]);
            ...
            $this->logger->error(\aportela\MusicBrainzWrapper\CoverArtArchive::class . '::get - Error: cached data for identifier is empty', [$mbId]);
